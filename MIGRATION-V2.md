...
| **P2b-full** | swarm `install` deploy step now routes through `deploy.sh` when the unified tree is present: renders the bundle, writes `unified/.env.<env>` (domain/TLS + config defaults), runs the assembler; keeps deploy-traefik (provides traefik-public) + create-secrets + seed. Legacy `deploy-swarm.sh` kept as fallback. ⚠️ **needs a VM run to validate** (typechecks; 83 tests green). compose install unchanged (delegates to `deploy`). Follow-up: skip legacy `generate-uifusion-config.sh` on unified. | **done (swarm), pending VM validation** |
